...
export const HouseholdSyncModal: React.FC<HouseholdSyncModalProps> = ({
  ...
  return (
    <div 
      className={`fixed inset-0 z-50 flex  items-end sm:items-center justify-center p-0 sm:p-4 animate-in fade-in duration-150 ${isGlassTheme(currentTheme) ? 'backdrop-blur-sm bg-slate-900/15 ' : 'bg-slate-900/60 '}`}
    ...
    >
      ...
      >
        ...
        <div className={`px-4 pt-2.5 sm:px-5 shrink-0 ${isGlassTheme(currentTheme) ? 'bg-transparent' : theme.isDark ? 'bg-slate-900' : 'bg-white'}`}>
          <div className={`p-1 rounded-xl flex gap-1 border ${isGlassTheme(currentTheme) ? 'bg-white/20 border-white/30 ' : theme.isDark ? 'bg-slate-800/80 border-slate-700/60' : 'bg-slate-100/90 border-slate-200/60'}`}>
            <button

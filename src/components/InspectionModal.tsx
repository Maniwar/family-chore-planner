      ...
      >
        ...
        >
          <div className="flex items-center space-x-3 min-w-0 flex-1">
            <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-2xl bg-white/10  flex items-center justify-center text-lg sm:text-xl font-bold shrink-0 shadow-[inset_0_1px_1.5px_rgba(255,255,255,0.8)] border border-white/40">
              🔍
            </div>
        ...
        <div className="p-4 sm:p-6 space-y-4 overflow-y-auto flex-1">
          ...
          <div className={`flex items-center justify-between p-3.5 rounded-2xl border ${
            isGlass 
              ? 'bg-white/20  border-white/20 shadow-[inset_1px_1.5px_0_rgba(255,255,255,1),0_4px_16px_rgba(31,38,135,0.06)]' 
            ...
            <div className="text-right">
              <span className={`px-3 py-1 rounded-xl text-xs font-black ${
                isGlass 
                  ? 'bg-amber-100/90 text-amber-950 border border-amber-300/80 shadow-[inset_0_1px_1px_rgba(255,255,255,0.9)] '
                  : 'bg-amber-100/80 dark:bg-amber-950 text-amber-900 dark:text-amber-200 border border-amber-200/90 dark:border-amber-800 '
              }`}>
          ...
          {log?.completedNote && (
            <div className={`p-3.5 rounded-2xl border ${
              isGlass 
                ? 'bg-amber-50/60  border-amber-200/90 shadow-[inset_0_1px_1.5px_rgba(255,255,255,0.8)]' 
                  ...
                  return (
                    <div
                      ...
                      className={`flex items-center gap-3 p-3 rounded-xl cursor-pointer transition-all text-xs sm:text-sm min-h-[44px] touch-target select-none active:scale-[0.98] ${
                        isChecked
                          ? isGlass
                            ? 'bg-emerald-500/20 text-emerald-950 font-bold border border-emerald-400/90  shadow-[inset_0_1px_1.5px_rgba(255,255,255,0.8),0_4px_12px_rgba(16,185,129,0.15)]'
                            : 'bg-emerald-100/80 text-emerald-950 font-semibold border border-emerald-300  shadow-xs'
                          : isGlass
                          ? 'bg-white/10 text-slate-900 hover:bg-white/20 border border-white/30  shadow-[inset_1px_1.5px_0_rgba(255,255,255,0.4)]'
                      ...
                      <div className="shrink-0 flex items-center justify-center">
                        {isChecked ? (
                          ...
                          <div className={`w-6 h-6 rounded-lg ${
                            isGlass
                              ? 'border-1.5 border-white/20 bg-white/60  shadow-[inset_0_1px_1.5px_rgba(255,255,255,0.95)]'
          ...
          <div>
            ...
            <div className={`mt-2 p-2.5 rounded-2xl border text-center text-xs font-bold ${currentRatingDesc.tone} ${isGlass ? ' shadow-[inset_0_1px_1.5px_rgba(255,255,255,0.7)]' : ''}`}>
          ...
          <div className={`p-3.5 rounded-2xl border ${
            isGlass 
              ? 'bg-gradient-to-br from-amber-50/60 to-orange-50/60  border-amber-200/90 shadow-[inset_1px_1.5px_0_rgba(255,255,255,0.95),0_4px_16px_rgba(245,158,11,0.1)]' 
            ...
            <div className="flex items-center justify-between mb-2">
              ...
              <div className={`text-xl sm:text-2xl font-black text-amber-900 dark:text-amber-200 px-3.5 py-1 rounded-2xl border border-amber-300 dark:border-amber-700 shadow-xs ${
                isGlass ? 'bg-white/85  shadow-[inset_0_1px_1.5px_rgba(255,255,255,1)]' : 'bg-white dark:bg-slate-900'
              }`}>
                {totalPoints} pts
              </div>
            </div>
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className={`w-9 h-9 rounded-xl text-slate-700 dark:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-700 border border-amber-200 dark:border-amber-700 shadow-xs flex items-center justify-center font-bold active:scale-95 cursor-pointer ${
                    isGlass ? 'bg-white/85  shadow-[inset_0_1px_1px_rgba(255,255,255,0.9)]' : 'bg-white dark:bg-slate-800'
                  }`}
                >
          ...
          <div>
            ...
            <textarea
              ...
              className={`w-full text-xs p-3 rounded-2xl border ${
                isGlass 
                  ? 'bg-white/20  border-white/30 text-slate-900 placeholder:text-slate-600 shadow-[inset_0_1px_2px_rgba(0,0,0,0.05)]' 
              ...
              {feedbackPresets.map((preset, idx) => (
                <button
                  ...
                  className={`text-[11px] px-2.5 py-1.5 rounded-xl transition-colors text-left font-semibold cursor-pointer ${
                    isGlass 
                      ? 'bg-white/55 hover:bg-white/80 text-slate-800 border border-white/20  shadow-[inset_0_1px_1px_rgba(255,255,255,0.8)]' 
        ...
        <div className={`p-3 sm:p-4 border-t ${
          isGlass 
            ? 'border-white/30 bg-white/10  shadow-[0_-4px_16px_rgba(0,0,0,0.05)]' 
          ...
          <button
            ...
            className={`inline-flex items-center justify-center gap-1.5 py-2.5 px-4 rounded-2xl text-xs font-bold transition-colors cursor-pointer min-h-[44px] ${
              isGlass
                ? 'bg-rose-500/15 hover:bg-rose-500/25 text-rose-950 border border-rose-400/50 shadow-[inset_0_1px_1.5px_rgba(255,255,255,0.6)] '
